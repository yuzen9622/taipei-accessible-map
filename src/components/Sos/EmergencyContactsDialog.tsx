...
export default function EmergencyContactsDialog({
  ...
  useEffect(() => {
    if (!open || !contacts.some((c) => c.bindStatus === "pending")) {
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            contacts.map((contact) => (
              <div
                key={contact._id}
                className="flex items-center justify-between gap-2 p-2.5 rounded-lg border border-border/60"
              >
                ...
                <button
                  ...
                  className="shrink-0 h-7 w-7 rounded-full hover:bg-muted flex items-center justify-center text-muted-foreground"
